...
def load_spectrum(filepath, smooth=False):
    ...
    wave = np.asarray(spec_table["wave_cal"]).astype(float).ravel()
...
def get_coords(f):
    ...
    ra = header["RA_OBJ"] * u.deg
    dec = header["DEC_OBJ"] * u.deg
...
with PdfPages("MMT/figures/mmt_spec_inspect.pdf") as pdf:
    for i, f in enumerate(coadd_files[:5]):
        ...
        outer_gs = GridSpec(4, 1, height_ratios=[3, 1.2, 1.2, 1.2], hspace=0.3)  # top block, zoom row 1, zoom row 2
        ...
        zoom_gs3 = outer_gs[3].subgridspec(1, 3, wspace=0.3)
        ax_zoom3 = [fig.add_subplot(zoom_gs3[i]) for i in range(3)]
        ...
        ax_zoom[0].legend(loc="upper left")
